...
void main() {
  ...
  group('AppColorTokens — surfaces', () {
    ...
    test(
        'should expose three distinct raised surfaces when reading the '
        'dark set', () {
  ...
  group('AppColorTokens — accents', () {
    test(
        'should expose the three QuestLoggd accents when reading the dark '
        'set', () {
    ...
    test(
        'should pair a single green with the black label ink when reading '
        'the dark set', () {
      expect(colors.green, const Color(0xFF4CAF50));
      expect(colors.inkDark, const Color(0xFF000000));
    });

    test(
        'should keep violet out of the surface and accent tokens when '
        'scanning them', () {
  ...
  group('AppStatusTokens', () {
    test('should expose exactly the six schema statuses when reading the set',
        () {
    ...
    test('should carry the specified hue for each status when reading them',
        () {
  ...
  group('AppTypeTokens', () {
    test(
        'should use Space Grotesk w700 for the display steps when reading '
        'them', () {
    ...
    test(
        'should use Inter at w400 or w500 for the UI steps when reading '
        'them', () {
      for (final token in <AppTextToken>[
        type.body,
        type.meta,
    ...
    test('should expose the five primary steps when reading their metrics', () {
      ...
      expect(type.body.style.height, 1.5);
    ...
    test('should expose the three smaller steps when reading their metrics',
        () {
    ...
    test(
        'should carry uppercase intent on zone label and pill only when '
        'reading the scale', () {
    ...
    test(
        'should leave the value untouched through format when the token is '
        'not uppercase', () {
  ...
  group('AppRadiusTokens', () {
    ...
    test('should not add the pending one-off radii when reading the scale', () {
      ...
      for (final pending in <double>[20, 5, 38, 44]) {
  ...
  group('AppMotionTokens', () {
    ...
    test('should expose the screen-transition easing when reading the curves',
        () {
  ...
  group('AppTokens — copyWith and lerp', () {
    test(
        'should return an equal-valued instance when copyWith takes no '
        'arguments', () {
    ...
    test(
        'should replace only the named group when copyWith takes one '
        'argument', () {
    ...
    test(
        'should return a fully populated AppTokens when lerping between '
        'two instances', () {
      final other = AppTokens.dark.copyWith(
        color: AppColorTokens.dark.copyWith(
          canvas: const Color(0xFF000000),
        ),
